...
func (f *Fetcher) Fetch(ctx context.Context) error {
	...
	for _, src := range sources {
		...
		go func(source Source) {
			...
			if err != nil {
				log.Printf("[ERROR] fetching items from source %s: %v", err)
				return
			}
...
func (f *Fetcher) itemShouldBeSkipped(item model.Item) bool {
	...
	for _, keyword := range f.filterKeywords {
		titleContainerKeyword := strings.Contains(strings.ToLower(item.Title), keyword)

		if categoriesSet.Contains(keyword) || titleContainerKeyword {
			return true
		}
	}
	return false
